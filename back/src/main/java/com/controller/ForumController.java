...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 司机
            //级联表
            SijiEntity siji = sijiService.selectById(forum.getSijiId());
            if(siji != null){
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setSijiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//                            forumEntity.setSijiId(Integer.valueOf(data.get(0)));   //司机 要改的
